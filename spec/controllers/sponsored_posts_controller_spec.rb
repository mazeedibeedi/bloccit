...
RSpec.describe SponsoredPostsController, type: :controller do
  ...
  describe "POST #create" do
    it "increases the number of sponsored posts by 1" do
      expect{post :create, topic_id: my_topic.id, sponsored_post: {title: RandomData.random_sentence, body: RandomData.random_paragraph, price: rand(1000)}}.to change(SponsoredPost,:count).by(1)
    end
  ...
  describe "PUT #update" do
    it "updates sponsored post with expected attributes" do
      ...
      sponsored_post_instance = assigns(:sponsored_post)
      expect(sponsored_post_instance.title).to eq new_title
      expect(sponsored_post_instance.body).to eq new_body
      expect(sponsored_post_instance.price).to eq new_price
    end
